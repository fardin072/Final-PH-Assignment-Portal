...
import { CreateAssignmentRequest, Assignment as AssignmentType } from "@/lib/types";
...
export async function GET() {
  try {
    ...
    const formattedAssignments: AssignmentType[] = assignments.map((assignment) => ({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, deadline }: CreateAssignmentRequest = await request.json();
